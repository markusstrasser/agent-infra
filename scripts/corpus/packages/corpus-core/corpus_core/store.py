...
The store root is an explicit dependency. Callers construct a
``CorpusStore(root=...)`` at their process/config boundary and pass that handle
through every read/write path. There is deliberately no module-level default:
an omitted root is the production-footgun this package exists to avoid.
...
class CorpusStore:
    """Explicit handle for one corpus filesystem root."""
    ...
    def derive_paper_id(
        ...
        *,
        ...
        pid = derive_paper_id(doi=doi, pmid=pmid, pdf_sha=pdf_sha)
        if check_collision and doi:
            self._check_doi_collision(pid, doi)
        return pid

    def _check_doi_collision(self, pid: str, raw_doi: str) -> None:
        """Fail closed if pid already exists with a different DOI."""
        meta_path = self.paper_path(pid) / "metadata.json"
    ...
    def exists(self, paper_id: str) -> bool:
        return (self.paper_path(paper_id) / "metadata.json").exists()

    def iter_papers(self) -> Iterator[str]:
    ...
    def write_metadata(self, paper_id: str, metadata: dict) -> None:
        p = self.paper_path(paper_id)
    ...
    def update_metadata(self, paper_id: str, **fields) -> dict:
        rec = self.get(paper_id)
        rec.metadata.update(fields)
        rec.metadata["last_updated"] = _now()
        self.write_metadata(paper_id, rec.metadata)
        return rec.metadata

    def register_revision(self, paper_id: str, new_pdf_path: Path) -> "RevisionResult":
