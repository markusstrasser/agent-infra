...
def _annotate_some(n: int) -> list[str]:
...
def test_happy_path_clean_replay(corpus_root):
    """N annotations → replay matches live, no drift."""
    ids = _annotate_some(3)
...
def test_replay_returns_diff_dataclass(corpus_root):
    _annotate_some(1)
    diff = verify_replay_matches_current()
    assert isinstance(diff, ReplayDiff)


def test_replay_in_place_requires_confirm(corpus_root):
    _annotate_some(1)
    with pytest.raises(ValueError, match="confirm=True"):
        replay_in_place(confirm=False)


def test_replay_in_place_confirm_rebuilds_db(corpus_root):
    _annotate_some(2)
...
def test_replay_detects_missing_when_db_diverges(corpus_root):
...
def test_replay_detects_byte_drift_via_annotation_id_mismatch(corpus_root):
...
def test_replay_to_temp_graph_returns_unlinkable_path(corpus_root, tmp_path):
    _annotate_some(1)
    target = replay_to_temp_graph()
...
def test_replay_handles_empty_store(corpus_root):
