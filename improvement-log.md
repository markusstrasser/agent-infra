...
- **Root cause:** system-design
- **Status:** [measured-once] — crash/protocol parity already solved (`codex_hook_compat.py`). Semantic-degradation detection is the open gap; bar is recurs-2+-sessions, this is instance 1 (cosmetic). PROMOTE the static-lint extension on next sighting. Cruft eradication DONE this session (orchestrator confirmed dead: deleted `archived_orchestrator.py`, dead agent-state.json/stop-failures.jsonl writes, 3 Codex orphan hooks; agent-infra@df9afe0).
