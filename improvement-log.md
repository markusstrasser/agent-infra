...
The events `executemany` shows `sqlite3_step → sqlite3BtreeNext` (a B-TREE SCAN, not an index probe) per source — the SAME stack as the original 18h hang (PID 1897). Measured ~24s/source (20 imports / 8 min), so the 2005-source historical backfill would take ~13h holding the lock — impractical. Indexes are present and correct (`idx_events_run_seq` UNIQUE, event_id PK), so the scan is elsewhere: candidates are the self-referential FK (`parent_event_id REFERENCES events(event_id)`) enforced per-insert, or `_refresh_session_denorm`'s correlated subqueries per source. **This is the real reason the historical gap can't be backfilled, and likely the true root of the original "hang" (FTS was only an amplifier).** Fix: profile the insert (EXPLAIN/`PRAGMA foreign_keys=OFF` in --bulk; batch the denorm; or check FK index coverage), get per-source under ~1s, then the backfill is minutes. HIGH. Forward indexing works around it via `--since-days 60 --limit-sources 40` (cheap bounded runs), so the loop recovers going forward; the historical May-4→Jun-8 gap waits on this fix.
- **Status:** [ ] proposed
