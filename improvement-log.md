...
### [2026-06-08] INDEXER_PERSOURCE_INSERT_SLOW: ~24s/source on the 11GB/2.76M-row DB — backfill impractical (agent-infra)
The events `executemany` shows `sqlite3_step → sqlite3BtreeNext` (a B-TREE SCAN, not an index probe) per source — the SAME stack as the original 18h hang (PID 1897). Measured ~24s/source (20 imports / 8 min), so the 2005-source historical backfill would take ~13h holding the lock — impractical. Indexes are present and correct (`idx_events_run_seq` UNIQUE, event_id PK), so the scan is elsewhere. **FK hypothesis RULED OUT this session:** `PRAGMA foreign_keys=OFF` during --bulk gave no speedup (~1.7 imports/min, same as FK-on) — the self-ref `parent_event_id` FK is NOT the bottleneck. Prime remaining suspect: `_refresh_session_denorm` (runs MIN/MAX/COUNT correlated subqueries over the session's events AFTER every source) — or something in the per-source write path. **This is the real reason the historical gap can't be backfilled, and likely the true root of the original "hang" (FTS was only an amplifier).** Next: profile `_refresh_session_denorm` in isolation (it's the most likely O(N) scan); batch it / make it incremental; get per-source under ~1s → backfill becomes minutes. HIGH. Forward indexing works around it via `--since-days 60 --limit-sources 40` (cheap bounded runs), so the loop recovers going forward; the historical May-4→Jun-8 gap waits on this fix.
- **Status:** [ ] proposed
