...
- **Proposed fix:** [architectural — DEFERRED, do NOT build yet] A hook-parity contract smoke (analog of `mcp_contract_smoke.py`): feed each Codex hook a representative payload + assert no dependency on Claude-only state files / absent events. Principle: Codex hooks read Codex-native state (rollout JSONL / sqlite), never Claude-only `/tmp/claude-cockpit-*` or `agent-state.json`.
- **Root cause:** system-design
- **Status:** [measured-once] — governance bar is recurs-2+-sessions; this is instance 1, mostly cosmetic. PROMOTE to build on next hook-parity-drift sighting. Cruft eradication (dead agent-state.json writes + 2 orphan Codex hooks) gated on user confirming the orchestrator data-path is dead, not parked.
