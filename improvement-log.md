...
- **Three independent kill legs (first two N-independent):** (1) **Pre-Build** — the TAA scorer already exists as `scripts/trace-faithfulness.py` (+ `reasoning-audit`/`tool-trajectory`/`pushback-index`/`thesis-challenge`/`fold-detector`); (2) **ephemeral substrate** — 4/5 cleanest human-labeled gold sessions rotated off disk AND out of `agentlogs.db`, so session-level calibration is infeasible without label-time feature snapshots; (3) **TAA falsified** — entity thought↔action alignment is flat/INVERTED on surviving labels (pos 0.078 ≥ neg 0.050) and length-normalized fab/errors-ignored invert (positives better). The one survivor (repeated-tool rate, pos 7.97 vs neg 3.86/100tc) is the existing TRACER signal for thrashing/build-then-undo — out of scope.
- **Root reason TAA can't work here:** the dominant documented failure is **coherent rationalization** (thought AGREES with the wrong action → alignment HIGH on a bad call). That's a **semantic** failure; the constitution already routes it to cross-model `/critique`. Deterministic trace-alignment structurally cannot catch it.
- **Kept:** existing report-only detectors. **Deferred (real gap):** session-loop's missing per-enforcer success metric is a *consumption* problem, not a new-scorer problem. **Reversibility:** low-irreversibility — build narrowly if a real incident ever evades both the report-only fleet and `/critique`.
- **Status:** [~] killed (pre-registered valid outcome)
- **Source:** this session; sibling of the [2026-06-01] session-quality retirement below (same "don't build a session scorer without a measured gap" lineage).
...
- **Status:** [x] implemented for three named hooks — intel `afcbc71` covers `pretool-adversarial-review-gate.py` (paste-able disposition-link line + model-review.py invocation), `pretool-deployment-evidence-gate.py` (downgrade-to-TRACKING escape hatch alongside fill-the-items path), `pretool-falsifier-disposition-gate.py` (ready-to-paste conviction_journal YAML template with today's date injected). **Broader audit completed 2026-05-11** — sampled 14 BLOCK-emitting intel hooks + 12 BLOCK-emitting skills hooks beyond the original three. All sampled hooks already include literal fix text: `console-helper-gate` names the canonical module import, `buy-quality-gate` names the required section + sub-checks, `buy-workup-required-gate` names the file-naming convention, `coverage-floor-gate` names the install command, `hardcoded-path-gate` names the literal `tools.lib.paths` exports, `numeric-sanity-gate` names the literal `## Numeric Sanity Override` section, `plan-protect` names the literal override token, `multiagent-commit-guard` gives explicit alternative commands, `stop-subagent-synthesis-gate` says "Use the Write tool to create the file." Intel's `data-guard` invocation in `.claude/settings.json` already overrides the generic default with "Use tools/setup_duckdb.py to rebuild." **Conclusion:** the dup-read loops were primarily the agent re-reading the *target file* to find an insertion point, not the *hook message* failing to prescribe. The cross-hook design principle remains valid as an authoring rubric for new hooks; no further retrofit edits warranted.
...
### [2026-06-07] SILENT_DEGRADATION: Codex hooks consume Claude-only runtime state (agent-infra)
- **Session:** agent-infra (this session) — Codex CLI Ghostty tab-title build
- **Evidence:** `~/.codex/hooks/stop-notify.sh` emitted `$0.00 0%` on every Codex Stop because it read `context_pct`/`cost` from `/tmp/claude-cockpit-{session_id}`, a file ONLY Claude's `statusline.sh` writes (never runs in Codex). Fixed this session to read the rollout JSONL (`last_token_usage.input_tokens / model_context_window`) via lsof-resolved session→rollout mapping; `codex-tab-title.sh` is the new reference pattern. Two further hooks are dead in Codex: `rate-limit-backoff.sh` + `billing-alert.sh` fire on a `StopFailure` event absent from Codex's hook model (0 refs in `~/.codex/hooks.json`). All three write `~/.claude/agent-state.json`, whose only consumer is `archived_orchestrator.py` (archived 2026-04-24, no launchd job) — generation-without-consumption per `consumption-over-autonomy.md`.
- **Failure mode:** Structural gap. MCP parity (Claude↔Codex) is synced+verified by `codex_parity_sync.py`; HOOK parity is hand-mirrored with zero verification, so hooks silently degrade (wrong output) or go dead (unwired event) when Codex's event model / runtime state differs from Claude's.
- **Correction (same session):** the contract-checker I first proposed as "deferred" ALREADY EXISTS — `scripts/codex_hook_compat.py` (+ `test_codex_hook_compat.py`), in-flight uncommitted this session. It runs `.codex/hooks.json` hooks with Codex-shaped payloads and flags two failure classes: non-JSON stdout, and accidental non-zero exits. **It would NOT have caught this bug:** `stop-notify.sh` exited 0 with valid protocol and merely read an absent Claude-only state file (`/tmp/claude-cockpit-*`) → wrote a wrong OSC title to /dev/tty. That is *semantic* degradation, not a crash/protocol violation.
- **Residual gap (the real, precise finding):** `codex_hook_compat.py` covers crash/protocol parity but NOT silent semantic degradation — a hook that runs clean but depends on Claude-only runtime state (cockpit/agent-state files) that never exists in Codex. Candidate extension: a static lint flagging Codex-wired hooks that read `/tmp/claude-cockpit-*` / `agent-state.json` / other Claude-statusline-written paths. Principle: Codex hooks read Codex-native state (rollout JSONL / sqlite), never Claude-only files — `codex-tab-title.sh` is the reference.
- **Root cause:** system-design
- **Status:** [resolved — one-off, lint NOT justified] Measured the full surface this session: of 58 wired Codex hooks, exactly 2 reference Claude-only artifacts (`stop-notify.sh`, `tab-color.sh`) and BOTH are comment/write-only — zero remaining live reads of Claude-statusline-only state. The semantic-degradation class is a confirmed single instance (`stop-notify`, fixed), not a pattern. Do NOT build the static-lint extension — there is nothing for it to catch; that would be enforcing against a non-problem. Crash/protocol parity stays covered by `codex_hook_compat.py`. Cruft eradication DONE (orchestrator confirmed dead: deleted `archived_orchestrator.py` + dead agent-state.json/stop-failures.jsonl writes + 3 Codex orphan hooks; agent-infra@df9afe0). Reopen only if a *new* Claude-only read appears in a Codex hook.

- [x] [2026-06-08] rule:checkable-claims-carry-probes — checkable "breaking/blocked" verdicts must carry their probe; downstream re-runs before acting on a skip. blast_radius=local, verifier=null (semantic, on generative backlog). Evidence: google-genai 2.x over-skip + cache-flag dismissal this session.

### [2026-06-08] GOALS_GAP: no objective for cost/leverage — reactive loops blind to friction-that-passes (agent-infra)
The generative principle maximizes declining-supervision (autonomy). It is structurally blind to work that *succeeds but is far short of possible* — the test loop was ~100x slow for months with zero error signal. Fix is architectural, not a metric: generalize the now-fast `orphan_check` standing-consumer pattern to the telemetry already collected-and-unconsumed (tool_latency → slow-command tail; eval scores → quality regress; hook-roi → dead hooks) — deterministic signal → one line in doctor/gov-report → a `/leverage` candidate. NEVER an LLM-proposal loop (vetoed session_quality class). Proposal touches GOALS (a second standing objective: declining cost-per-outcome) → human-gated.
- **Status:** [ ] proposed

### [2026-06-08] DEAD_INFRA: runlogs.db (0 bytes since 2026-04) still read by 4 scripts — silent empty output (agent-infra)
`reasoning-audit.py`, `ops.py`, `token-baseline.py`, `agent_surface.py` read `~/.claude/runlogs.db` (dead; live store is `agentlogs.db`). `reasoning-audit` is now wired into gov-report, so it reports on emptiness. session-forensics rule fixed this session (pointed at agentlogs). Remaining: schema-aware repoint each script to agentlogs.db OR retire. Scoped task — do NOT blind-repoint (schemas differ = iatrogenic).
- **Status:** [ ] proposed

### [2026-06-08] IATROGENIC: whole-tree linters block scoped commits on unrelated in-flight changes (genomics)
`lint_silent_fallbacks.py` (and peers) in the pre-commit lint the working tree, not staged content, so any unrelated uncommitted change blocks ALL commits → forces `--no-verify` (which skips the real gates too). Hit 3+ times this session. Fix: scope pre-commit linters to staged paths like `qa-checks.sh` already does (`git diff --cached`). Investigate which linters are whole-tree first.
- **Status:** [ ] proposed
...
### [2026-06-08] CODEX_INDEXING: event_id collision — mechanism CONFIRMED + HANDLED (agent-infra)
Mechanism (confirmed, not guessed): the events `executemany ... ON CONFLICT(run_id, seq) DO UPDATE SET event_id = excluded.event_id` REASSIGNS event_id; on a re-import / cross-boundary session continuation the seq↔event_id mapping shifts, so the UPDATE tries to set an event_id that ALREADY exists on another (run_id, seq) row → events.event_id PK violation, aborting the whole source. NOT multi-file (UUIDs don't repeat), NOT fork/resume (no id≠filename), NOT a hash collision (sha1/64-bit, 2.76M events). The actual colliders were legacy pre-May-4 files re-touched because discovery had no recency bound.
- **HANDLED (committed):** per-row fallback on batch IntegrityError — skip the one colliding event, log its existing location, keep the rest of the source (no more whole-source abort). + recency bound so legacy files aren't re-touched at all. Forward codex indexing now works.
- **Status:** [x] implemented (collision no longer blocks indexing). Superseded as the blocker by the PERF finding below.

### [2026-06-08] INDEXER_DENORM_SCAN: per-source denorm scanned all user_message events — FIXED (agent-infra)
The per-source slowness (~24s/src, `sqlite3BtreeNext` scan, same stack as the original 18h hang) was `_refresh_session_denorm` = 74% of `_write_parsed` (cProfile, not guessed). Its `first_message` subquery (`WHERE kind='user_message'`) had no supporting index → planner used `idx_events_kind` and SCANNED all 45,995 user_message events + temp sort for EVERY session. FK hypothesis ruled out first (foreign_keys=OFF no speedup). Fix (migration 004, cdb2d3b): `CREATE INDEX events(run_id,kind,seq)` + ANALYZE (required) + 1GiB cache/8GiB mmap. Measured `_write_parsed` 5.36s→0.94s, denorm 3.22s→**0.002s** (1600x). **This was the true root of the original "hang" (FTS only amplified it).** Steady-state forward indexing is now fast.
- **Status:** [x] implemented.

### [2026-06-08] INDEXER_PARSE_HEAVY_SOURCES: a few codex files take minutes to parse, stall bulk runs (agent-infra)
After the denorm fix the bulk backfill is STILL slow end-to-end (~1-6 imports/min) — a NEW, smaller bottleneck: a minority of codex rollouts have millions of lines and take 5-7 min in `parse_source` (CPU-bound pure Python). The DB watchdog (sqlite progress handler) does NOT bound parse (no sqlite call to interrupt), so these giants stall a run. Net: the 2005-source historical gap can't be one bulk job; it drains incrementally via the bounded launchd (`--limit-sources 40`, ~40/run over days). Fix for the giants: a SIGALRM-based parse watchdog (a Python signal CAN preempt pure-Python parse, unlike a blocked C sqlite call) → skip+log the pathological source; or cap parse at N lines. MED — forward indexing works; only the gap-drain speed + a few giant sessions are affected.
- **Status:** [ ] proposed

### [2026-06-08] INDEXER_NO_RECENCY_BOUND: unbounded DB growth — recency bound SHIPPED, prune still open (agent-infra)
SHIPPED: `--since-days` discovery bound (rolling window); launchd now runs `--since-days 60 --limit-sources 40`. STILL OPEN: an age-out PRUNE so the 11GB DB actually shrinks (the bound stops growth at the read side but old rows persist) — pairs with the perf fix above (a smaller DB is also a faster DB).
- **Status:** [ ] proposed (prune only; bound done)
