...
### [2026-06-08] CODEX_INDEXING_BLOCKED: cross-source event_id collision — mechanism UNCONFIRMED (agent-infra)
New codex sources fail the events executemany with `UNIQUE constraint failed: events.event_id`: an event_id being inserted already exists in the table under a DIFFERENT (run_id, seq), and `ON CONFLICT(run_id, seq)` doesn't cover the event_id PK, so the whole source fails. Net effect: **post-May-4 codex sources don't index; codex data stays at May 4 until fixed.** HIGH — user's recent-session data.
- **Partial fixes SHIPPED (a7b2c9d):** batch-dedupe (intra-source collisions) + rollback-guard (one bad source now fails ISOLATED instead of aborting the whole vendor — so the indexer no longer hangs/cascades, it just skips colliders). These are correct but insufficient.
- **Hypotheses RULED OUT this session (don't re-probe):** (1) multi-file sessions sharing a session_id — session UUIDs do NOT repeat across rollout filenames. (2) fork/resume where `session_meta.id` ≠ filename UUID — no such mismatch found in 300 post-May-4 files. The earlier "22/120 colliders" probe was FLAWED (it counted already-indexed-and-skippable sources as colliders; verified one such source has a matching success import record and WOULD be skipped).
- **Next session — DO forensics, don't guess:** instrument the actual failing INSERT (wrap the executemany, on IntegrityError print the colliding event_id + its existing run_id/seq vs the new one), find ONE true failing source, and trace why two distinct (run_id,seq) share an event_id. THEN fix (likely: make event_id incorporate enough to be globally unique, or make the insert resilient to PK collision via per-source clean delete). Verify `test_ingest_idempotent` still holds. Then backfill the ~2005-source gap.
- **Status:** [ ] proposed
