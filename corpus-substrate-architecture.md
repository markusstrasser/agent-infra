...
- **`corpus-core` lives under `agent-infra/scripts/`** but is a genuine shared library (4 consumers). Cohesion would improve in a dedicated `corpus` library repo with its own version history; functionally identical (editable-install either way). Deliberate cross-repo refactor, not urgent.
- **The append-only ledger is not git-tracked.** `~/Projects/corpus` is a plain data dir, so `annotations.jsonl` (629 KB — tiny, text) has no git history — yet the constitution treats "every correction is a commit / git log is the audit trail" as the moat. Tracking the ledger (gitignoring `graph.duckdb`, `*.pdf`, `parsed.*/`) would make that literal. The cheap high-value half of the seam above.
